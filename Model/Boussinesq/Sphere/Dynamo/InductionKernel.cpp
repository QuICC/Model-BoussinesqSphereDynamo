...
InductionKernel::InductionKernel() : IPhysicalKernel() {}
...
void InductionKernel::setMagnetic(std::size_t name,
...
void InductionKernel::setVelocity(std::size_t name,
   Framework::Selector::VariantSharedVectorVariable spField)
{
   // Safety assertion
   assert(this->mScalars.count(name) + this->mVectors.count(name) == 0);
...
void InductionKernel::compute(Framework::Selector::PhysicalScalarField& rNLComp,
      ...
      std::visit(
         [&](auto&& m, auto&& v)
         {
            Physical::Cross<FieldComponents::Physical::THETA,
               FieldComponents::Physical::PHI>::set(rNLComp, m->dom(0).phys(),
               v->dom(0).phys(), this->mInduction);
         },
         this->vector(this->name()), this->vector(this->mVelName));
      break;
      ...
      std::visit(
         [&](auto&& m, auto&& v)
         {
            Physical::Cross<FieldComponents::Physical::PHI,
               FieldComponents::Physical::R>::set(rNLComp, m->dom(0).phys(),
               v->dom(0).phys(), this->mInduction);
         },
         this->vector(this->name()), this->vector(this->mVelName));
      break;
      ...
      std::visit(
         [&](auto&& m, auto&& v)
         {
            Physical::Cross<FieldComponents::Physical::R,
               FieldComponents::Physical::THETA>::set(rNLComp, m->dom(0).phys(),
               v->dom(0).phys(), this->mInduction);
         },
         this->vector(this->name()), this->vector(this->mVelName));
      break;
