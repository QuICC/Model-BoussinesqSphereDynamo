/** 
 * @file MomentumKernelICBL.cpp
 * @brief Source of physical space kernel for the Momentum equation (Inertia, Coriolis, Buoyancy, Lorentz)
 */
...
namespace Kernel {
   ...
   MomentumKernelICBL::MomentumKernelICBL()
      : IPhysicalKernel()
   {
   }
   ...
   std::size_t MomentumKernelICBL::name() const
   {
      return this->mName;
   }

   void MomentumKernelICBL::setVelocity(std::size_t name, Framework::Selector::VariantSharedVectorVariable spField)
   {
      // Safety assertion
   ...
   void MomentumKernelICBL::setTemperature(std::size_t name, Framework::Selector::VariantSharedScalarVariable spField)
   {
      // Safety assertion
      assert(this->mScalars.count(name) + this->mVectors.count(name) == 0);
   ...
   void MomentumKernelICBL::setMagnetic(std::size_t name, Framework::Selector::VariantSharedVectorVariable spField)
   {
      // Safety assertion
      assert(this->mScalars.count(name) + this->mVectors.count(name) == 0);
   ...
   void MomentumKernelICBL::init(const MHDFloat inertia, const MHDFloat coriolis, const MHDFloat buoyancy, const MHDFloat lorentz)
   {
   ...
   void MomentumKernelICBL::setMesh(std::shared_ptr<std::vector<Array> > spMesh)
   {
   ...
   void MomentumKernelICBL::compute(Framework::Selector::PhysicalScalarField& rNLComp, FieldComponents::Physical::Id id) const
   {
      ///
      ...
      std::visit([&](auto&& s){Physical::SphericalBuoyancy::add(rNLComp, id, s->dom(0).res(), this->mRadius, s->dom(0).phys(), this->mBuoyancy);}, this->scalar(this->mTempName));
